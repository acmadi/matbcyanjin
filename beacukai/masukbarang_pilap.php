...
$(function(){
	$('#dg').datagrid({
		...
		singleSelect:true,
		idField:'CAR',
		url:'<?php echo $basedir ?>models/masukbarang_listrpt.php',
		pagination:'true',
		...
		columns:[[  
			...
			{title:'Bukti Penerimaan<br>Barang',colspan:2,width:55,align:"center",
			...
			{field:'NmTuj',title:'Pemasok/Pengirim',rowspan:2,width:40}, 
...
function cari(){						
	$('#dg').datagrid({  
		url:"<?php echo $basedir ?>models/masukbarang_listrpt.php?dtdari="+$('#dtdari').combo('getValue')+"&dtsampai="+$('#dtsampai').combo('getValue')+"&DokKdBc="+$('#DokKdBc').val()
	});
	$('#dg').datagrid('load');
}
...
function showPrint(){						
	...
	window.open("masukbarang_rpt.php?dtdari="+dtdari+"&dtsampai="+dtsampai+"&DokKdBc="+DokKdBc);
}
...
function toExcel(){						
	...
	window.open("masukbarang_rpt.php?dtdari="+dtdari+"&dtsampai="+dtsampai+"&DokKdBc="+DokKdBc+"&cetak="+3);
}
</script>
</head>
<body>
	...
	<div id="tb" style="padding:5px;height:auto">
        <div>
        	Jenis Dokumen: <select id="DokKdBc" name="DokKdBc" style="width:80px;" tabindex="2">
                  <option value=""></option>
                  <?php 
					$q="SELECT * FROM jenis_dok WHERE KdJnsDok IN ('1','8','6','5') ORDER BY UrJnsDok";
					$run = $pdo->query($q);
		  		    $rs = $run->fetchAll(PDO::FETCH_ASSOC);
					foreach ($rs as $r){
