...
$(function(){
	$('#dg').datagrid({
		...
		singleSelect:true,
		idField:'CAR',
		url:'<?php echo $basedir ?>models/bc41/bc41_listrpt.php',
		columns:[[  
			...
			{field:'detail',title:'Detail',rowspan:2,width:25,align:'center',
				formatter:function(value,row,index){
					var det = '<a href="#" onclick="window.open(\'bc41_rpt.php?CAR='+row.CAR+'\', \'_blank\');"><img src="<?php echo $basedir ?>themes/icons/txt.png"></a> <a href="#" onclick="window.open(\'bc41_rpt.php?CAR='+row.CAR+'&cetak=3\', \'_blank\');"><img src="<?php echo $basedir ?>themes/icons/xls.png"></a>';
					return det;					
				}
			}
		],[						
...
function cari(){						
	$('#dg').datagrid({  
		url:"<?php echo $basedir ?>models/bc41/bc41_listrpt.php?dtdari="+$('#dtdari').combo('getValue')+"&dtsampai="+$('#dtsampai').combo('getValue')
	});
	$('#dg').datagrid('load');
}
</script>
